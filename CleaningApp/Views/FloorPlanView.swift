...
struct FloorPlanView: View {
    ...
    private var sortedRooms: [Room] { home.rooms.sorted { $0.sortOrder < $1.sortOrder } }
                ...
                VStack(alignment: .leading, spacing: 20) {
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                        ...
                        Button { showAddRoom = true } label: {
                            ...
                            .frame(maxWidth: .infinity).frame(height: 90).background(Color(.systemGray6))
                            .clipShape(RoundedRectangle(cornerRadius: 14))
                            .overlay(RoundedRectangle(cornerRadius: 14).stroke(style: StrokeStyle(lineWidth: 1, dash: [5])).foregroundStyle(Color(.systemGray3)))
                        }.buttonStyle(.plain)
                    }.padding(.horizontal)
                }.padding(.vertical)
            }
...
struct RoomGridCard: View {
    ...
    private var pendingCount: Int { room.tasks.filter { $0.isActive && ($0.isOverdue || $0.isDueToday) }.count }
    var body: some View {
...
struct RoomDetailView: View {
    ...
    private var activeTasks: [CleaningTask] { room.tasks.filter { $0.isActive }.sorted { $0.nextDueDate < $1.nextDueDate } }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if selectedTab == 0 { taskList } else { FixtureListView(room: room) }
        }
        .navigationTitle(room.name).navigationBarTitleDisplayMode(.large)
        .toolbar {
            ToolbarItem(placement: .primaryAction) {
                if selectedTab == 0 { Button { showAddTask = true } label: { Image(systemName: "plus") } }
            }
        }
        .sheet(isPresented: $showAddTask) { AddTaskSheet(home: room.home!, preselectedRoom: room) }
    }

    private var taskList: some View {
        List {
            if activeTasks.isEmpty {
                ContentUnavailableView("タスクがありません", systemImage: "sparkles", description: Text("＋ボタンからタスクを追加しましょう"))
                    .listRowBackground(Color.clear)
            } else {
                ForEach(activeTasks) { task in NavigationLink(destination: TaskDetailView(task: task)) { TaskListRow(task: task) } }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 4) {
                ...
                if task.frequency.supportsWeekdays && !task.weekdays.isEmpty { Text("(\(task.weekdaysLabel))").font(.caption) }
                if task.estimatedMinutes > 0 { Text("·"); Image(systemName: "clock").font(.caption2); Text("約\(task.estimatedMinutes)分").font(.caption) }
    ...
    var body: some View {
        List {
            Section("基本情報") {
                ...
                if task.frequency.supportsWeekdays && !task.weekdays.isEmpty { LabeledContent("曜日", value: task.weekdaysLabel) }
            ...
            Section("使用用品") {
                ...
                if task.supplies.isEmpty { Text("用品が未登録です").font(.subheadline).foregroundStyle(.tertiary) }
            }
            ...
            Section("実施記録（直近5件）") {
                ...
                if task.logs.isEmpty { Text("まだ記録がありません").font(.subheadline).foregroundStyle(.tertiary) }
            }
        }
        .navigationTitle(task.title).navigationBarTitleDisplayMode(.large)
        .toolbar { ToolbarItem(placement: .primaryAction) { Button { showComplete = true } label: { Label("完了", systemImage: "checkmark.circle") }.tint(.teal) } }
...
struct AddRoomSheet: View {
    ...
    private let iconOptions = ["sofa","fork.knife","bed.double","shower","door.left.hand.open","toilet","books.vertical","drop","washer","refrigerator","stove","window.horizontal"]
        ...
        NavigationStack {
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 6), spacing: 12) {
                        ForEach(iconOptions, id: \.self) { iconName in
                            Button { icon = iconName } label: {
                                Image(systemName: iconName).font(.title3).frame(width: 40, height: 40)
                                    .background(icon == iconName ? Color.teal.opacity(0.15) : Color(.systemGray6))
                                    .foregroundStyle(icon == iconName ? .teal : .secondary).clipShape(RoundedRectangle(cornerRadius: 8))
                            }.buttonStyle(.plain)
                        }
                    }.padding(.vertical, 4)
                }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("追加") { let room = Room(name: name, icon: icon, sortOrder: home.rooms.count); room.home = home; context.insert(room); try? context.save(); dismiss() }
...
struct AddTaskSheet: View {
    ...
    private var roomFixtures: [Fixture] { (selectedRoom?.fixtures ?? []).sorted { $0.name < $1.name } }
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("設定") {
                    ...
                    if frequency.supportsWeekdays {
                        VStack(alignment: .leading, spacing: 8) {
                            Text(frequency == .weekly ? "毎週の曜日" : "隔週の曜日").font(.subheadline).foregroundStyle(.secondary)
                ...
                if !roomFixtures.isEmpty {
                    Section {
                        ForEach(roomFixtures) { fixture in
                            FixtureSelectionRow(fixture: fixture, isSelected: selectedFixtureIDs.contains(fixture.id)) {
                                if selectedFixtureIDs.contains(fixture.id) { selectedFixtureIDs.remove(fixture.id) }
                                else { selectedFixtureIDs.insert(fixture.id); if title.trimmingCharacters(in: .whitespaces).isEmpty { title = fixture.name + "の清掃" } }
                            }
                        }
                    } header: {
                        HStack { Text("対象の設備・器具"); Spacer(); Text("\(selectedFixtureIDs.count)件選択").font(.caption).foregroundStyle(.secondary) }
                    } footer: { Text("複数選択可。選択した設備に紐づくタスクとして管理されます").font(.caption) }
                }
                if !allSupplies.isEmpty {
                    Section("使用用品") {
                        ForEach(allSupplies) { supply in
                            HStack { Text(supply.name); Spacer(); if selectedSupplyIDs.contains(supply.id) { Image(systemName: "checkmark").foregroundStyle(.teal) } }
                                .contentShape(Rectangle())
                                .onTapGesture { if selectedSupplyIDs.contains(supply.id) { selectedSupplyIDs.remove(supply.id) } else { selectedSupplyIDs.insert(supply.id) } }
                        }
                    }
                }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("追加") { saveTask() }
                        .disabled(title.trimmingCharacters(in: .whitespaces).isEmpty || selectedRoom == nil).fontWeight(.semibold)
                }
            }
    ...
    private func saveTask() {
        ...
        let task = CleaningTask(title: title, frequency: frequency, weekdays: Array(selectedWeekdays), intervalDays: 7, nextDueDate: nextDueDate, estimatedMinutes: estimatedMinutes, notes: notes)
        task.room = room
        ...
        context.insert(task); try? context.save(); dismiss()
    }
}
...
struct FixtureSelectionRow: View {
    ...
    var body: some View {
        Button(action: onTap) {
            HStack(spacing: 12) {
                Image(systemName: fixture.icon).font(.title3).foregroundStyle(isSelected ? .teal : .secondary).frame(width: 28)
                VStack(alignment: .leading, spacing: 2) {
                    Text(fixture.name).font(.subheadline).fontWeight(.medium).foregroundStyle(.primary)
                    if !fixture.parts.isEmpty { Text("パーツ \(fixture.parts.count)件").font(.caption).foregroundStyle(.secondary) }
                }
                Spacer()
                Image(systemName: isSelected ? "checkmark.circle.fill" : "circle").foregroundStyle(isSelected ? .teal : Color(.systemGray3)).font(.title3)
            }
        }.buttonStyle(.plain)
    }
}
    ...
    var body: some View {
        HStack(spacing: 6) {
            ForEach(Weekday.allCases) { weekday in
                ...
                Button {
                    if isSelected { selectedWeekdays.remove(weekday.rawValue) } else { selectedWeekdays.insert(weekday.rawValue) }
                } label: {
                    Text(weekday.label).font(.subheadline).fontWeight(.medium).frame(maxWidth: .infinity).padding(.vertical, 8)
                        .background(isSelected ? Color.teal : Color(.systemGray6)).foregroundStyle(isSelected ? .white : .secondary)
