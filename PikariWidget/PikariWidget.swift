...
private extension Color {
    ...
    static let pikariAccent = Color(red: 0.37, green: 0.51, blue: 0.95)
}
...
struct TodayTasksWidgetView: View {
    ...
    private var allUrgent: [WidgetTaskEntry] {
        (data.overdueTasks + data.todayTasks)
    }
...
struct TodayTasksWidget: Widget {
    let kind: String = "com.hiroki.CleaningApp.todayTasks"
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: TodayTasksProvider()) { entry in
            TodayTasksWidgetView(data: entry.data)
...
struct NextTaskProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<NextTaskEntry>) -> Void) {
        ...
        let entry = NextTaskEntry(date: .now, task: task)
        completion(makeTimeline(entry: entry))
    }
}
...
struct NextTaskWidget: Widget {
    let kind: String = "com.hiroki.CleaningApp.nextTask"
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: NextTaskProvider()) { entry in
            NextTaskWidgetView(task: entry.task)
                .containerBackground(.clear, for: .widget)
        }
                ...
                if parts.isEmpty {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ForEach(Array(parts.prefix(4))) { part in
                            HStack(spacing: 6) {
                                Circle()
                                    .fill(part.stockCount == 0 ? Color.red : Color.orange)
                                    .frame(width: 6, height: 6)
                                VStack(alignment: .leading, spacing: 0) {
                                    Text(part.name)
                                        .font(.caption).fontWeight(.medium)
                                        .foregroundStyle(.white).lineLimit(1)
                                    Text(part.fixtureName)
                                        .font(.caption2).foregroundStyle(.white.opacity(0.5))
                                }
                                Spacer()
...
struct LowStockWidget: Widget {
    let kind: String = "com.hiroki.CleaningApp.lowStock"
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: LowStockProvider()) { entry in
            LowStockWidgetView(parts: entry.parts)
                .containerBackground(.clear, for: .widget)
        }
...
struct WeeklyProgressProvider: TimelineProvider {
    func placeholder(in context: Context) -> WeeklyProgressEntry {
        WeeklyProgressEntry(date: .now, done: 0, total: 0)
    }
    func getSnapshot(in context: Context, completion: @escaping (WeeklyProgressEntry) -> Void) {
        let data = WidgetDataStore.load()
        completion(WeeklyProgressEntry(date: .now, done: data.weeklyDoneCount, total: data.weeklyTotalCount))
    }
    func getTimeline(in context: Context, completion: @escaping (Timeline<WeeklyProgressEntry>) -> Void) {
        let data = WidgetDataStore.load()
        let entry = WeeklyProgressEntry(date: .now, done: data.weeklyDoneCount, total: data.weeklyTotalCount)
        completion(makeTimeline(entry: entry))
    }
}
...
struct WeeklyProgressWidgetView: View {
    ...
    private var progress: Double {
        total == 0 ? 0 : min(1.0, Double(done) / Double(total))
    }
...
struct WeeklyProgressWidget: Widget {
    let kind: String = "com.hiroki.CleaningApp.weeklyProgress"
    var body: some WidgetConfiguration {
